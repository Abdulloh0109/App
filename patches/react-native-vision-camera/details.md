...
- Reason (2) — **`NSGenericException` from an unbalanced capture-session configuration**: `-[AVCaptureSession stopRunning] stopRunning may not be called between calls to beginConfiguration and commitConfiguration`, thrown from `CameraSession.checkIsActive` on the camera queue.
  ...
  - **Preview layer during teardown**: Releasing an attached `AVCaptureVideoPreviewLayer` detaches it from the session inside its own `beginConfiguration()`/`commitConfiguration()`, on whichever thread drops the last reference. The `didSetProps` closures retain `CameraView`, but the teardown closure in `deactivateCameraSession()` does not, so React Native could release the view, and with it the preview layer, on the main thread while the teardown `configure` was about to call `stopRunning()`. The teardown closure now holds the preview layer until that `configure` call returns, so the detach can only happen after `stopRunning()`. It still does not retain `CameraView`, so the "Local correction" above is unaffected.
...
- E/App issue:
  ...
  - Fix 2: https://github.com/Expensify/App/issues/97103, https://github.com/Expensify/App/issues/99850
